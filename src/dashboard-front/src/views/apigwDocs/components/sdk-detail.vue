<template>
  <div v-if="sdk" class="data-box wrapper">
    <div class="row-item mb10">
      ...
      <div class="value">
        <span class="column-value" v-bk-overflow-tips>{{sdk.sdk_name || '--'}}</span>
      </div>
    </div>
    <div class="row-item mb10">
      <div class="key">
      ...
      <div class="value">
        <span class="column-value" v-bk-overflow-tips>{{sdk.sdk_version_number || '--'}}</span>
      </div>
    </div>
    ...
    <div class="row-item mb10">
      ...
      <div class="value">
        <bk-popover placement="top" width="600" :disabled="!sdk.sdk_download_url">
          <main class="column-value vm">{{sdk.sdk_download_url || '--'}}</main>
          <template #content>
            <div style="white-space: normal;word-break: break-all;">
              {{ sdk.sdk_download_url }}
            </div>
          </template>
        </bk-popover>
      </div>
      <aside class="suffix">
        <i
          ...
          v-if="sdk.sdk_download_url"
          v-bk-tooltips="t('复制')"
          @click="copy(sdk.sdk_download_url)"
        >
        </i>
        <i
    ...
    <div class="row-item mb10">
      ...
      <div class="value">
        <bk-popover placement="top" width="600" v-if="sdk.sdk_install_command">
          <main class="column-value vm">{{sdk.sdk_install_command}}</main>
          <template #content>
            <div style="white-space: normal;word-break: break-all;">
              {{ sdk.sdk_install_command }}
            </div>
          </template>
        </bk-popover>
        <span v-else>--</span>
      ...
      <aside class="suffix">
        <i
          ...
          v-if="sdk.sdk_install_command"
          v-bk-tooltips="t('复制')"
          @click="copy(sdk.sdk_install_command)"
        >
        </i>
      </aside>
    </div>
    ...
    <template v-if="isApigw">
      <div class="row-item mb10">
        ...
        <div class="value">
          <span
            class="column-value"
            v-bk-tooltips.top="{ content: sdk.resource_version ?? '--', allowHTML: false }">
            {{ sdk.resource_version ?? '--'}}
          </span>
        </div>
      </div>

      <div class="row-item mb10" v-if="sdk.stage?.name">
        <div class="key">
          <span class="column-key">
            {{ t('版本已发环境') }}:
          </span>
        ...
        <div class="value">
          <span class="column-value" v-bk-tooltips.top="sdk.stage?.name">{{sdk.stage?.name || '--'}}</span>
        </div>
      </div>
    </template>
  </div>
